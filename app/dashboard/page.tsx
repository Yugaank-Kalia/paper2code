					...
					{papers.length > 0 ? (
						<div className='grid gap-4 sm:grid-cols-2'>
							{papers.map((paper) => (
								<Card
									key={paper.id}
									className='flex flex-col h-full border-border/50 bg-card/50 backdrop-blur-xs transition-all duration-300 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5'
								>
									...
									<CardFooter className='pt-0 grid grid-cols-2 gap-2 w-full mt-auto'>
										<Button
											variant='outline'
											className='col-span-1 cursor-pointer gap-2'
											asChild
										>
											<Link
												href={`/dashboard/pdf/${paper.id}`}
										...
										>
											<Link
												href={`/dashboard/chunks/${paper.id}`}
											>
						...
						<div className='flex flex-col items-center justify-center py-24 text-center'>
							...
							<p className='text-muted-foreground mb-8 max-w-sm'>
								...
								chunk it for semantic search, and convert it to
								code.
							</p>
							<UploadPaperDialog text='Upload Paper' />
						</div>
